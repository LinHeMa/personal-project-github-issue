...
import {
  ...
  addSortCondition,
  resetLabelCondition
} from '../../slices/labelSlice/LabelListActionSlice';
      ...
      {labelPop && (
        <div>
          ...
          <div
            className={
              renderType !== 'Filters'
                ? ` fixed top-8 left-[20px] right-[20px] z-20  mx-auto flex max-h-[400px] min-h-[400px] flex-col overflow-y-auto rounded-xl border border-solid bg-white opacity-100 group-last:rounded-b-lg sm:absolute sm:min-h-fit sm:w-[300px] sm:border-stone-300 md:right-auto md:left-[5px] md:top-[25px] lg:right-[0px] lg:left-auto`
                : `fixed left-[20px] top-12 right-[20px] z-20  mx-auto flex min-h-[800px] flex-col rounded-xl border border-solid bg-white opacity-100 group-last:rounded-b-lg sm:absolute sm:left-0 sm:right-auto sm:top-12 sm:min-h-fit sm:w-[300px]  sm:border-stone-300 md:right-auto md:left-0 md:top-12  lg:left-0 lg:top-12`
            }
          >
              ...
              {renderType === 'Label' &&
                labels?.map((label) => {
                  return (
                    <div key={label.id}>
                      <div
                        className={`hover:bg-[#0969da] hover:text-[#ffffff] flex flex-wrap items-center  justify-start  border-b-[1px] border-solid border-stone-300 p-[16px] text-[14px] font-semibold leading-[21px]  ${
                          _.includes(
                            _.toLower(label.name),
                            _.trim(filterInput.toLowerCase(), ' '),
                          )
                            ? ''
                            : _.includes(
                                _.toLower(label.description),
                                _.trim(filterInput.toLowerCase(), ' '),
                              )
                            ? ''
                            : 'hidden'
              ...
              {renderType === 'Assignee' &&
                assignees?.map((assignee) => {
                  return (
                    <div
                      key={assignee.id}
                      className={`hover:bg-[#0969da] hover:text-[#ffffff] flex items-center justify-start p-[16px] text-[14px] font-semibold leading-[21px]  
                      ${
                        _.includes(
              ...
              {renderType === 'Sort' &&
                sortList?.map((item, index) => {
                  return (
                    <div
                      key={index}
                      className='flex items-center justify-start p-[16px] text-[14px] font-semibold leading-[21px] hover:bg-[#0969da] hover:text-[#ffffff]'
                      onClick={() => {
                        setIsClicked((prev) => !prev);

...
const Item: React.FC<ItemProps> = ({ data }) => {
  ...
  const [, setValue] = useSessionStorage('issueNumber', 0);
  ...
  return (
    ...
    >
      <div className='flex h-max cursor-pointer border-b-[1px] border-solid   border-stone-300  hover:bg-[#f6f8fa] sm:border sm:border-t-[0px] '>
              ...
              ? assignees.map((assignee, index) => {
                  return (
                    <img
                      ...
                      className={`hidden h-[20px] w-[20px] rounded-full transition-all duration-300 ease-in-out md:block ${
                        assignees.length !== 1 &&
                      ...
                      ${clsx({
                        ...
                        'md:hidden': index < assignees.length - 3,
                      })}
                      `}
                    />
                  );
